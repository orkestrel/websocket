...
| API                                 | Kind  | Summary                                                                                                                               |
| ----------------------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `WEBSOCKET_GUID`                    | const | Names the RFC 6455 GUID concatenated to a client's `Sec-WebSocket-Key` before the accept hash.                                        |
| `WEBSOCKET_VERSION`                 | const | Names the WebSocket protocol version this wrapper speaks (`Sec-WebSocket-Version: 13`).                                               |
| `WEBSOCKET_OPCODE_TEXT`             | const | Names the text frame opcode — a UTF-8 payload (RFC 6455 §5.6).                                                                        |
| `WEBSOCKET_OPCODE_BINARY`           | const | Names the binary frame opcode — a raw byte payload (RFC 6455 §5.6).                                                                   |
| `WEBSOCKET_OPCODE_CONTINUATION`     | const | Names the continuation frame opcode — the next fragment of an open data message (RFC 6455 §5.4).                                      |
| `WEBSOCKET_OPCODE_CLOSE`            | const | Names the close frame opcode — a control frame ending the connection (RFC 6455 §5.5.1).                                               |
| `WEBSOCKET_OPCODE_PING`             | const | Names the ping frame opcode — a control frame the peer must answer with a pong (RFC 6455 §5.5.2).                                     |
| `WEBSOCKET_OPCODE_PONG`             | const | Names the pong frame opcode — a control frame answering a ping (RFC 6455 §5.5.3).                                                     |
| `WEBSOCKET_READY_CONNECTING`        | const | Names the ready state for a connecting WebSocket (before the handshake completes).                                                    |
| `WEBSOCKET_READY_OPEN`              | const | Names the ready state for an open WebSocket (the handshake completed; frames flow).                                                   |
| `WEBSOCKET_READY_CLOSING`           | const | Names the ready state for a closing WebSocket (a close frame was sent or received).                                                   |
| `WEBSOCKET_READY_CLOSED`            | const | Names the ready state for a closed WebSocket (the socket ended).                                                                      |
| `WEBSOCKET_CLOSE_NORMAL`            | const | Names the normal-closure status code (RFC 6455 §7.4.1) — the default `close` code.                                                    |
| `WEBSOCKET_CLOSE_PROTOCOL`          | const | Names the protocol-error status code (RFC 6455 §7.4.1) — a framing/state rule was violated.                                           |
| `WEBSOCKET_CLOSE_UNSUPPORTED`       | const | Names the unsupported-data status code (RFC 6455 §7.4.1) — the endpoint received a data type it cannot accept.                        |
| `WEBSOCKET_CLOSE_INVALID`           | const | Names the invalid-frame-payload-data status code (RFC 6455 §7.4.1) — for example non-UTF-8 text or an unparseable close reason.       |
| `WEBSOCKET_CLOSE_TOO_BIG`           | const | Names the message-too-big status code (RFC 6455 §7.4.1) — a reassembled message exceeded the payload cap.                             |
| `WEBSOCKET_MAX_PAYLOAD`             | const | Names the default cap on both an inbound frame's declared length and a reassembled message's total byte count (100 MiB).              |
| `WEBSOCKET_CLOSE_TIMEOUT_MS`        | const | Names the default close-handshake timeout in milliseconds — how long `close` waits for the peer's echo.                               |
| `WEBSOCKET_CONTROL_MAX_LENGTH`      | const | Names the maximum control-frame payload length in bytes (RFC 6455 §5.5).                                                              |
| `WEBSOCKET_CLOSE_REASON_MAX_LENGTH` | const | Names the maximum UTF-8 close-reason length after the two-byte status code.                                                           |
| `WEBSOCKET_FAIL_TIMEOUT_MS`         | const | Names the flush grace in milliseconds a validation-breach close frame is given before the hard teardown fallback destroys the socket. |

### Types

| API                      | Kind      | Summary                                                                                                                                                                                          |
| ------------------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `WebSocketReadyState`    | type      | Represents a WebSocket ready state — the browser-compatible `0` connecting, `1` open, `2` closing, and `3` closed.                                                                               |
| `WebSocketFrame`         | interface | Represents a parsed RFC 6455 frame — the structured result of decoding one frame off the wire.                                                                                                   |
| `WebSocketEncodeOptions` | interface | Represents the options for `encodeWebSocketFrame` — how a frame is masked on the wire.                                                                                                           |
| `WebSocketErrorCode`     | type      | Represents the subject a `WebSocketError` names as refused — `OPTION`, `LIMIT`, `CLOSE`, or `FRAME`.                                                                                             |
| `NodeWebSocketEventMap`  | type      | Represents the event map a `NodeWebSocketInterface` emitter carries — `open`, `message`, `close`, `error`, `ping`, and `pong`.                                                                   |
| `NodeWebSocketOptions`   | interface | Represents the options for `createNodeWebSocket` — the upgraded `socket`, the `key` that selects server or client mode, and the listeners, caps, and cancellation signal the wrapper runs under. |
| `NodeWebSocketInterface` | interface | Represents the behavioral contract a server-native WebSocket exposes over a raw upgraded socket — the `emitter` and `readyState` data members plus `send`, `ping`, `close`, and `destroy`.       |
